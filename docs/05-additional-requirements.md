...
| --- | --- | --- | --- |
| AR-1 | **[REQUIRED]** | PARTIAL | **A new profile can set its picture as part of creating the profile.** Today a `participant` row is created lazily on first portal visit with only `eventId`/`userId`/`displayName` (`lib/services/portal.ts:64`), and the headshot is set afterwards through a separate `setHeadshot` call (`lib/services/portal.ts:290`). `profileGaps()` nudges for the missing image (`lib/services/portal.ts:306`) — a nudge is not the same as the flow asking for it at creation time. The headshot cannot join `profileSchema` (`lib/services/portal.ts:141`) because Server Action bodies cap at 1 MB, so the create flow has to compose the existing upload route with the profile write, not merge them |
...
| AR-2a | **[IMPORTANT]** | OUTSTANDING | **Make the `file_blob` ceiling visible.** Now that Postgres is a supported production backend rather than a fallback of last resort, its limits should be stated where an operator will see them: image bytes inflate database size and every backup of it, and Hyperdrive/Workers response limits bound how large a served file can get. Document the practical ceiling and warn — at seed time or in the admin storage view — when a deployment is approaching it |
| AR-3 | **[REQUIRED]** | DEFECT | **A profile picture referenced by the public API must be fetchable.** `app/api/v1/_lib/queries.ts:317` and `lib/accelevents/sync.ts:164` both emit `${appUrl()}/api/files/{fileId}` as `headshotUrl`. **That route does not exist.** Every headshot URL the public API and the Accelevents push hand out is dead. Either build the route with the same structural gate the embed route uses (`app/embed/[slug]/headshot/[fileId]/route.ts:34`) or stop emitting the field |
| AR-4 | **[IMPORTANT]** | OUTSTANDING | **Validate and normalize uploaded images.** Current checks are a content-type prefix match and a 10 MB cap (`app/portal/[eventSlug]/upload/route.ts:16-20`). No dimension bound, no re-encode, no thumbnail. A 10 MB image is served at full size to every roster row and every embed |
| AR-5 | **[IMPORTANT]** | OUTSTANDING | **One headshot model, not two.** `participant.headshotFileId` (`db/schema.ts:678`) points at stored bytes; `contact.headshotUrl` (`db/schema.ts:1307`) is a plain external URL string on the CRM table. Two representations of the same concept will drift. Pick one and give the other a documented conversion |
| AR-6 | **[OPTIONAL]** | DEFECT | **Honor `S3_FORCE_PATH_STYLE`.** Documented in `.env.example` and set in `docker-compose.yml`, but `lib/storage/index.ts:76` hardcodes `forcePathStyle: true` and never reads it. Harmless with MinIO, wrong against any S3-compatible host that requires virtual-hosted addressing |
| AR-7 | **[EXCLUDED]** | — | **Presigned direct-to-bucket uploads.** Declined on purpose: routing every read through the app is what makes "only someone with a role on this event can download this deck" enforceable per request, and the R2 binding cannot presign at all (`lib/storage/index.ts:18-23`). Consequence to clean up — `@aws-sdk/s3-request-presigner` is a dependency that nothing imports |
