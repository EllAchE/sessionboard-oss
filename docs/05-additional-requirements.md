...
This began as a post-v1 goal because a model prompt alone is not an optimizer. The implemented
version adds the missing inputs, a deterministic scoring model, organizer-controlled weights, and
representative schedule evaluations while keeping every placement advisory.

| ID | Tag | Status | Requirement |
| --- | --- | --- | --- |
| AR-36 | **[IMPORTANT]** | SHIPPED | **The agenda builder optimizes for likely audiences and the physical venue.** It infers audience overlap from tracks, tags, personas, level, format, title and abstract text, so related talks such as energy-and-software and bioweapons-and-software are kept out of the same slot when the program has room to separate them. `submission.expected_attendance` and `participant.popularity_score` are organizer-owned inputs to the demand estimate; room capacity determines stage fit, and room floor informs the flow between related adjacent sessions. Six event-level weights — audience overlap, attendance forecast, speaker popularity, room fit, venue flow and compactness — are stored on the event, bounded from 0–100 and editable before every draft. Room, track and speaker conflicts remain hard constraints; every generated placement is revalidated and remains unapplied until the organizer accepts it. Deterministic evaluation fixtures cover overlapping software audiences, stage sizing, hard conflicts, venue flow, weight changes and repeatability (`lib/ai/agenda-optimizer.ts`, `app/admin/agenda/AiProposalDialog.tsx`) |
...
| [`03-plan.md`](03-plan.md) | Workstream ownership still applies: AR-1–AR-7 land in W2, AR-8–AR-18 in W5, AR-19–AR-27 in W7, AR-28–AR-29 in W3, AR-30–AR-34 in W6 on W5's send primitives, AR-35 in W4 (and crosses W0 for the one `event` column it adds), and AR-36 spans W4/W9 with its schema inputs routed through W0 |
