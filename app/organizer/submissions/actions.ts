...
async function run<T>(work: () => Promise<T>, path = '/organizer/submissions'): Promise<ActionResult<T>> {
  try {
...
export async function saveScorecardAction(input: {
  ...
  }, `/organizer/submissions/${input.submissionId}`);
}
...
export async function assignOneAction(
  ...
  return run(async () => {
    ...
    return { created };
  }, `/organizer/submissions/${submissionId}`);
}
