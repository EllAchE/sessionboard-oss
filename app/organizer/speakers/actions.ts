...
export async function setSpeakerStatusAction(
  ...
  try {
    ...
    revalidatePath('/organizer/speakers');
    revalidatePath(`/organizer/speakers/${participantId}`);
...
/**
 * `S-10`, the way in. The way out already existed — the portal shell's "Return to organizer" banner —
 ...
 * identity, so there is no organizer page left to render a toast on.
 */
