      ...
      render: (row) => (
        <span className={styles.person}>
          <Avatar
            name={row.name}
            size="sm"
            src={row.headshotFileId ? `/organizer/speakers/photo/${row.headshotFileId}` : undefined}
          />
          <span className={styles.personText}>
            <Link href={`/organizer/speakers/${row.id}`} className={styles.personName}>
              {row.name}
            </Link>
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        rows={rows}
        getRowId={(row) => row.id}
        onRowActivate={(row) => router.push(`/organizer/speakers/${row.id}`)}
        emptyState={
          speakers.length === 0
