...
const KIND_LABEL: Record<OrganizerTaskRow['kind'], string> = {
  form: 'Form',
...
const AUDIENCE_LABEL: Record<OrganizerTaskRow['audience'], string> = {
...
const SCOPE_LABEL: Record<OrganizerTaskRow['scope'], string> = {
  contact: '',
...
const COLUMNS: Array<DataTableColumn<OrganizerTaskRow>> = [
  {
    id: 'name',
    header: 'Task',
    width: '30%',
...
export function TasksIndex({
  ...
  const [editing, setEditing] = useState<OrganizerTaskRow | null>(null);
  const [editorOpen, setEditorOpen] = useState(false);
  const [confirming, setConfirming] = useState<OrganizerTaskRow | null>(null);
  ...
  const openEdit = (row: OrganizerTaskRow) => {
    setEditing(row);
    setEditorOpen(true);
  };
  ...
  const columns: Array<DataTableColumn<OrganizerTaskRow>> = canManage
    ? [
        ...COLUMNS,
        {
          id: 'actions',
