...
export const TEMPLATE_VARIABLES: TemplateVariable[] = [
  { path: 'event.name', description: 'Assembly name' },
  { path: 'event.dates', description: 'Assembly dates, e.g. 14–16 September 2026' },
  { path: 'event.timezone', description: 'Assembly timezone' },
  { path: 'event.venue', description: 'Forum or venue name' },
  { path: 'event.website', description: 'Assembly website URL' },
  { path: 'event.url', description: 'Public Cicero proclamation for the assembly' },
  { path: 'event.supportEmail', description: 'Magistrates’ dispatch address' },
  { path: 'speaker.name', description: 'Orator display name' },
  { path: 'speaker.firstName', description: 'First name only' },
  { path: 'speaker.email', description: 'Orator dispatch address' },
  { path: 'speaker.company', description: 'House or company' },
  { path: 'speaker.jobTitle', description: 'Office or title' },
  { path: 'speaker.pronouns', description: 'Pronouns' },
  { path: 'submission.title', description: 'Petition title' },
  { path: 'submission.ref', description: 'Petition reference, e.g. ABS-12' },
  { path: 'submission.status', description: 'Petition standing' },
  { path: 'submission.decisionNote', description: 'Note entered beside the council’s verdict' },
  { path: 'session.title', description: 'Scheduled oration title' },
  { path: 'session.ref', description: 'Oration reference, e.g. SESS-4' },
  { path: 'session.track', description: 'Programme theme' },
  { path: 'session.room', description: 'Chamber name' },
  { path: 'session.format', description: 'Oration format' },
  { path: 'session.startsAt', description: 'Opening hour in the assembly timezone' },
  { path: 'session.endsAt', description: 'Closing hour in the assembly timezone' },
  { path: 'session.calendarUrl', description: 'Personal fasti download link (C-3a)' },
  { path: 'tasks.count', description: 'Number of unsettled duties' },
  { path: 'tasks.list', description: 'Unsettled duties as a markdown list' },
  { path: 'tasks.next', description: 'Name of the next duty due' },
  { path: 'task.name', description: 'Duty name (reminder dispatches only)' },
  { path: 'task.dueAt', description: 'Duty due date (reminder dispatches only)' },
  { path: 'portal.url', description: 'Orator atrium URL' },
  { path: 'portal.link', description: 'One-click sealed link into the atrium' },
  { path: 'form.name', description: 'Scroll name (deadline reminders only)' },
  { path: 'form.closesAt', description: 'Scroll closing date (deadline reminders only)' },
  { path: 'form.url', description: 'Public scroll URL (deadline reminders only)' },
];
...
export const AUDIENCE_LABELS: Record<AudienceKind, string> = {
  all_speakers: 'Every orator with a petition',
  accepted_speakers: 'Accepted orators',
  pending_speakers: 'Orators awaiting the council’s verdict',
  declined_speakers: 'Orators whose petitions were declined',
  scheduled_speakers: 'Orators with an appointed hour',
  track: 'Orators in one theme',
  format: 'Orators in one format',
  outstanding_tasks: 'Orators with an unsettled duty',
  manual: 'Names chosen from the rolls',
};
...
export const DEFAULT_TEMPLATES: Array<{
...
}> = [
  {
    ...
    name: 'Petition entered in the rolls',
    subject: 'Cicero has recorded “{{submission.title}}”',
    bodyMarkdown: [
      'Salve {{speaker.firstName|friend}},',
      '',
      'Your petition **{{submission.title}}** ({{submission.ref}}) has entered the rolls of {{event.name}}.',
      '',
      'You may inspect or revise it from your private atrium:',
      '',
      '[Enter your orator atrium]({{portal.link}})',
      '',
      'A dispatch will follow when the council reaches its verdict.',
    ].join('\n'),
  },
  {
    key: 'submission.accepted',
    name: 'Petition accepted by decree',
    subject: 'The council has accepted your oration for {{event.name}}',
    bodyMarkdown: [
      'Salve {{speaker.firstName|friend}},',
      '',
      'The council has spoken: **{{submission.title}}** ({{submission.ref}}) is accepted for {{event.name}}.',
      '',
      '{{submission.decisionNote}}',
      '',
      'Before you take the rostra, please settle these duties:',
      '',
      '{{tasks.list}}',
      '',
      '[Enter your orator atrium]({{portal.link}})',
  ...
  },
  {
    ...
    name: 'Petition declined by decree',
    subject: 'The council’s verdict on your {{event.name}} petition',
    bodyMarkdown: [
      'Salve {{speaker.firstName|friend}},',
      '',
      'Thank you for placing **{{submission.title}}** before {{event.name}}. More worthy petitions reached the Forum than the fasti can hold, so the council cannot include this one.',
      '',
      '{{submission.decisionNote}}',
      '',
      'We hope you will return to the Forum with another argument.',
    ].join('\n'),
  },
  {
    key: 'session.invite',
    name: 'Summons to the fasti',
    subject: '{{session.title}} — {{session.startsAt}}',
    attachIcs: true,
    bodyMarkdown: [
      'Salve {{speaker.firstName|friend}},',
      '',
      'Your oration at {{event.name}} now has its hour in the fasti. Accept the attached calendar summons and it will enter your own calendar, then revise itself if the programme moves.',
      '',
      '- **Oration:** {{session.title}} ({{session.ref}})',
      '- **Begins:** {{session.startsAt}}',
      '- **Concludes:** {{session.endsAt}}',
      '- **Chamber:** {{session.room|to be proclaimed}}',
      '- **Theme:** {{session.track|—}}',
      '',
      'If your courier did not show accept and decline controls, [add the summons to your personal fasti]({{session.calendarUrl}}).',
    ].join('\n'),
  },
  {
    key: 'session.cancelled',
    name: 'Oration removed from the fasti',
    subject: 'Removed from the fasti: {{session.title}}',
    attachIcs: true,
    bodyMarkdown: [
      'Salve {{speaker.firstName|friend}},',
      '',
      '**{{session.title}}** ({{session.ref}}) has been struck from the {{event.name}} fasti. This dispatch also cancels the calendar summons sent earlier.',
      '',
      'If this decree surprises you, reply to this dispatch.',
    ].join('\n'),
  },
  {
    key: 'task.reminder',
    name: 'Duty reminder',
    subject: 'A duty remains: {{task.name}} for {{event.name}}',
    bodyMarkdown: [
      'Salve {{speaker.firstName|friend}},',
      '',
      'The duty **{{task.name}}** remains unsettled{{task.dueAt| }}.',
      '',
      'Every duty still open in your ledger:',
      '',
      '{{tasks.list}}',
      '',
      '[Enter your orator atrium]({{portal.link}})',
  ...
  {
    key: 'form.deadline',
    name: 'Unfinished petition reminder',
    subject: 'The {{event.name}} rolls close {{form.closesAt}}',
    bodyMarkdown: [
      'Salve {{speaker.firstName|friend}},',
      '',
      'An unfinished petition for **{{event.name}}** remains on your desk, and the rolls for {{form.name}} close on {{form.closesAt}}.',
      '',
      '[Finish and file your petition]({{form.url}})',
    ].join('\n'),
  },
];
...
export async function sendDecisionNotice(submissionId: string): Promise<SendOutcome> {
  ...
  if (row.status !== 'accepted' && row.status !== 'declined') {
    throw invalid('Only an accepted or declined petition has a verdict to dispatch');
  }
