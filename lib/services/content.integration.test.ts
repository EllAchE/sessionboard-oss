...
describe('revision numbers', () => {
  ...
  it("reruns the migration's own backfill to the same numbering", async () => {
    ...
    /**
     * The statement shipped in `0025`, run verbatim. It is what every existing database's history
    ...
    const migration = readFileSync(
      fileURLToPath(new URL('../../db/migrations/0025_bumpy_post.sql', import.meta.url)),
      'utf8',
    );
